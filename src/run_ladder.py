...
def run_one(model, fmt: str, log_dir: Path, items: str = ITEMS, max_tokens: int | None = None, epochs: int | None = None,
            ...
            temperature: float | None = None, no_thinking: bool = False) -> int:
    ...
    if no_thinking:
        cmd += ["-M", "reasoning_enabled=false"]  # REVIEW3.md run 2: OpenRouter reasoning: {enabled: false}
    print(f"\n### {model.id} [{fmt}] x{ep} items={items} host={host or model.host} quant={model.quant if host is None else '?'} max_tokens={mt} chart={chart} show_types={show_types} shuffle={shuffle} temperature={temperature} no_thinking={no_thinking}", flush=True)
...
def main() -> None:
    ...
    ap.add_argument("--chart", default="gen1", help="gen1 | none | modern | permuted (REVIEW2.md run 1 uses none; REVIEW3.md run 1 uses permuted)")
    ...
    ap.add_argument("--no-thinking", action="store_true", help="ask the host to disable reasoning (REVIEW3.md run 2)")
    a = ap.parse_args()
    ...
    for fmt in a.formats.split(","):
        for m in wanted:
            if run_one(m, fmt, log_dir, a.items, a.max_tokens, a.epochs, a.host, a.chart, a.show_types, a.shuffle, a.temperature, a.no_thinking) != 0:
